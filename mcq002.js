$(window).on('load', function() {
    ...
    if (video.getAttribute('title') == '00_Muscle_Intro_2023_b.mp4') {
        video.ontimeupdate = function() {showQuestion002()};
        //video.volume = 0.0;
    }
    // create MCQ div
    ...
    b1.onclick = function(){
        // submit function
        fbA.innerHTML = get_feedback002(iA, "A")
        fbB.innerHTML = get_feedback002(iB, "B")
        fbC.innerHTML = get_feedback002(iC, "C")
        fbD.innerHTML = get_feedback002(iD, "D")
    ...
    var q1_shown = false;
    function showQuestion002() {
        if ((!q1_shown) && (video.currentTime > 310)) {
            q1_shown = true;
            video.pause()
